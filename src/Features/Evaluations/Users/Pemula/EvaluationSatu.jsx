...
const EvaluationSatu = () => {
  ...
  useEffect(() => {
    ...
    // Filter data berdasarkan evaluationId
    const filteredQuestions =
      data?.filter(
        (item) =>
          item.source_type_id === 2 &&
          (evalId ? item.source_id === Number(evalId) : true)
      ) || [];
    setEvaluationQuestions(filteredQuestions);

    // Set current question evaluation jika ada
    const currentIndex = currentQuestionIndex ?? 0;
    setCurrentQuestionEvaluation(filteredQuestions[currentIndex]);
