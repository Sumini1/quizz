...
import { saveUserQuizProgress } from "../../../../AllQuestions/userQuizzes";
...
const QuizQuestions1 = () => {
   ...
   const { theme, getBorderClass, getButtonClass, middleTheme } = useTheme();
   ...
   const [isModalReferensiVisible, setIsModalReferensiVisible] =
     useState(false);
...
// state untuk menyimpan quizId yang sudah difilter berdasarkan quizId
   const [regularQuestionsQuizzes, setRegularQuestionsQuizzes] = useState([]);
   const [currentQuestionQuizzes, setCurrentQuestionQuizzes] = useState(null);
   ...
   useEffect(() => {
     ...
     const filteredQuestions =
       data?.filter(
         (item) =>
           item.source_type_id === 1 && item.source_id === Number(quizId)
       ) || [];
     setRegularQuestionsQuizzes(filteredQuestions);

     // Set current question quiz jika ada
     const currentIndex = currentQuestionIndex ?? 0;
     setCurrentQuestionQuizzes(filteredQuestions[currentIndex]);
   ...
   console.log(currentQuestionQuizzes);
   console.log("regularQuestionsQuizzes", regularQuestionsQuizzes);
   ...
   const handleSelectAnswer = (answer, index) => {
     ...
     const isCorrect = answer === currentQuestionQuizzes?.question_correct;
   ...
   const handleCheckAnswer = () => {
     if (selectedAnswer !== null) {
       ...
       const newProgress = Math.min(
         ((currentQuestionIndex + 1) / regularQuestionsQuizzes.length) * 100,
         100
       );
   ...
   const handleNextQuestion = () => {
     if (currentQuestionIndex < regularQuestionsQuizzes.length - 1) {
       ...
       const quizProgressData = {
         ...
         percentage_grade: Math.round(
           (score / regularQuestionsQuizzes.length) * 100
         ),
         point: score,
       ...
       navigate("/pemula/quiz-questions-1/final-scored", {
         state: {
           score,
           totalQuestions: regularQuestionsQuizzes.length,
        ...
        >
                  ...
                  style={{
                    width: `${
                      ((currentQuestionIndex + 1) /
                        regularQuestionsQuizzes.length) *
                      100
                    }%`,
                  }}
                ></div>
          ...
          <div className="flex flex-col  p-5">
            <div className="text-lg font-[500]">
              <p>{currentQuestionQuizzes?.question_text}</p>
            </div>
          </div>

          {/* Answer options */}
          <div
            className={` w-full gap-5 p-5 grid ${
              currentQuestionQuizzes?.question_answer?.some(
                (ans) => ans.length > 10
              )
                ? "grid-cols-1"
                : "grid-cols-2"
          ...
          >
            {currentQuestionQuizzes?.question_answer?.map((option, index) => (
              <h5
                key={index}
          ...
          {isModalVisible && (
            <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 ">
              <div
                className={`rounded-xl max-w-md mx-auto rounded-b-none w-full m-0 p-6 mt-[550px] items-center justify-center fixed bottom-0 ${
              ...
              >
                ...
                <div className="flex gap-5">
                  ...
                  >
                    {currentQuestionIndex < regularQuestionsQuizzes.length - 1
                      ? "Lanjut"
                      : "Selesai"}
                  </button>
                </div>
              ...
              >
                <div className="flex flex-col">
                  ...
                  <div className="text-[16px] overflow-y-scroll max-h-[400px] font-[300]">
                    <p className="mb-2">
                      {currentQuestionQuizzes?.explain_question}
                    </p>
                  </div>
              ...
              >
                <div className="flex flex-col">
                  ...
                  <div className="text-[16px] font-[300] mb-3 overflow-y-scroll max-h-[400px]">
                    <p>
                      {currentQuestionQuizzes?.paragraph_help ||
