...
const ModalNomorSatu = React.memo(({ isOpen, onClose, unitId, quizId }) => {
  ...
  const { data: units, error: unitsError } = useSelector(
    (state) => state.units
  );
  const { data: quizzes, error: quizzesError } = useSelector(
    (state) => state.quizzes
  );
  const { data: quizQuestions, error: quizQuestionsError } = useSelector(
  ...
  useEffect(() => {
    ...
    boxLabels.forEach((label) => {
      initialClicks[label] = 0;
    });
    setBoxClicks(initialClicks);
  }, [boxLabels]);

  useEffect(() => {
    if (selectedQuizId && quizQuestions?.length) {
      const relatedQuestion = quizQuestions.find(
        (question) => question.quiz_id === selectedQuizId
      );
      setSelectedQuizQuestionsId(relatedQuestion?.id || null);
    }
  }, [selectedQuizId, quizQuestions]);

  const handleBoxClick = useCallback(
    (box) => {
      if (quizBoxes[box]) {
        setSelectedQuizId(quizBoxes[box]);
        setBoxClicks((prev) => ({ ...prev, [box]: prev[box] + 1 }));
      }
    },
    [quizBoxes]
  );
  ...
  const completedQuizCount = 1; // Replace with actual logic to count completed quizzes
    ...
    >
      ...
      >
        ...
        <Link
          to={
            selectedQuizId
              ? `/page-satu-sirrah?quizId=${selectedQuizId}${
                  selectedQuizQuestionsId
